...
def hexagon(cx: float, cy: float, r: float, squash: float = 1.0) -> list[tuple[float, float]]:
    ...
    return [(cx + r * math.cos(math.radians(60 * i)),
             cy + r * math.sin(math.radians(60 * i)) * squash) for i in range(6)]
...
def render(out_size: int = 256, *, body_top=AZURE_LT, body_bottom=AZURE,
           letter=NAVY, supersample: int = 4) -> Image.Image:
    ...
    ImageDraw.Draw(im).line(cp + [cp[0]], fill=(12, 17, 24, 52),
                            width=max(1, int(size * 0.017)), joint="curve")

    w, h = size * 0.40, size * 0.485
    im.paste(Image.new("RGBA", (size, size), letter), (0, 0),
