...
def save_estimate(
    client: TestClient, order_id: int, items: list[dict], labor_rate=125.0, tax_rate=0.0825, actor="advisor"
) -> dict:
    return post(
        client,
...
def authorize(client: TestClient, order_id: int, status: str, approved_by: str, method: str = "phone") -> None:
    post(
        client,
        f"/api/orders/{order_id}/authorization",
        {
            "status": status,
...
def age_the_board() -> None:
    ...
    def days_ago(n: int) -> str:
        return (datetime.now(UTC) - timedelta(days=n, hours=1)).isoformat(timespec="seconds")
...
def main() -> None:
    ...
    post(client, "/api/staff", {"name": "Lou Grant", "role": "manager"})

    print("Building retail repair orders ...")

    # 1. Fresh estimate, nothing decided yet.
    cust, veh = customer_and_vehicle(
        client,
        "Maria Alvarez",
        "313-555-0101",
    ...
    cust, veh = customer_and_vehicle(
        ...
        "Tyrell Banks",
        "313-555-0102",
        "tyrell.banks@example.com",
        year=2020,
        make="Chevrolet",
        model="Equinox",
        vin="3GNAXKEV4LS123456",
        mileage=41250,
    )
    ...
    cust, veh = customer_and_vehicle(
        ...
        "Janelle Osei",
        "313-555-0103",
        "janelle.osei@example.com",
        year=2019,
        make="Honda",
        model="CR-V",
        vin="5J6RW2H86KL123456",
        mileage=55400,
    )
    ...
    save_estimate(
        ...
        [
            ...
            {
                "kind": "labor",
                "description": "Replace compressor + belt, evacuate/recharge system",
                "quantity": 3.0,
                "unit_price": 125.0,
            },
        ],
    ...
    cust, veh = customer_and_vehicle(
        ...
        "Devon Marsh",
        "313-555-0104",
        "devon.marsh@example.com",
        year=2016,
        make="Ford",
        model="F-150",
        vin="1FTEW1EG5GKE12345",
        mileage=112300,
    )
    ...
    save_estimate(
        ...
        [
            {
                "kind": "part",
                "description": "Full synthetic oil (6 qt)",
                "quantity": 6,
                "unit_price": 9.5,
                "unit_cost": 5.75,
    ...
    post(
        ...
        {"amount": invoice["total"], "method": "card", "actor": "advisor"},
    )
    ...
    cust, veh = customer_and_vehicle(
        ...
        "Whitney Cole",
        "313-555-0105",
        "whitney.cole@example.com",
        year=2015,
        make="Subaru",
        model="Outback",
        vin="4S4BSANC5F3123456",
        mileage=98700,
    )
    ...
    save_estimate(
        ...
        [
            ...
            {
                "kind": "labor",
                "description": "Remove/replace heads, gasket set, coolant flush",
                "quantity": 9.5,
                "unit_price": 125.0,
            },
        ],
    ...
    post(
        client,
        f"/api/invoices/{invoice5['id']}/payments",
        {"amount": round(invoice5["total"] / 2, 2), "method": "cash", "actor": "advisor"},
    )
    ...
    cust, veh = customer_and_vehicle(
        ...
        "Ken Ibarra",
        "313-555-0106",
        "ken.ibarra@example.com",
        year=2012,
        make="Nissan",
        model="Altima",
        vin="1N4AL2AP7CN123456",
        mileage=143200,
    )
    ...
    post(
        ...
        f"/api/orders/{o7['id']}/findings",
        {
            ...
            "items": [
                {
                    "kind": "part",
                    "description": "Sway bar end links (pair)",
                    "quantity": 1,
                    "unit_price": 58.0,
                    "unit_cost": 34.0,
                    "part_number": "SB-EL-220",
                },
                {
                    "kind": "part",
                    "description": "Front-left strut assembly",
                    "quantity": 1,
                    "unit_price": 210.0,
                    "unit_cost": 138.0,
                    "part_number": "ST-9910",
    ...
    save_estimate(
        ...
        [
            ...
            {
                "kind": "part",
                "description": "Brake pads + rotors, all 4",
                "quantity": 1,
                "unit_price": 0,
                "unit_cost": 240.0,
    ...
    ro_r3 = post(
        client,
        "/api/orders",
        {"concern": "Recon prep: full service, tires", "segment": "recon", "recon_vehicle_id": r3["id"]},
    )
    save_estimate(
        client,
        ro_r3["id"],
    ...
    buyer, _ = customer_and_vehicle(
        client, "Ashley Pratt", "313-555-0210", "ashley.pratt@example.com", year=2015, make="Chevrolet", model="Malibu"
    )
    patch(
        client,
        f"/api/recon/vehicles/{r4['id']}",
        {
            "status": "sold",
            "sale_price": 6995.0,
            "sale_date": "2026-07-15",
            "sale_customer_id": buyer["id"],
        },
    )
    ...
    patch(
        client, f"/api/recon/vehicles/{r5['id']}", {"status": "sold", "sale_price": 4500.0, "sale_date": "2026-04-20"}
    )
    ...
    we1 = post(
        ...
        "/api/we-owe",
        {
            "customer_id": cust["id"],
            "vehicle_id": veh["id"],
            "description": "Install aftermarket running boards (customer supplied)",
            "category": "accessory",
    ...
    ro_we1 = post(
        ...
        "/api/orders",
        {
            "concern": "Install running boards (customer-supplied)",
            "segment": "we_owe",
            "we_owe_id": we1["id"],
        },
    )
    ...
    we2 = post(
        ...
        "/api/we-owe",
        {
            "customer_id": cust["id"],
            "vehicle_id": veh["id"],
            "description": "Touch up paint on rear bumper from sale prep",
            "category": "cosmetic",
    ...
    post(
        client,
        f"/api/we-owe/{we2['id']}/payments",
        {"amount": 50.0, "method": "cash", "note": "Deposit toward outside body shop cost", "actor": "advisor"},
    )
    ...
    we4 = post(
        ...
        "/api/we-owe",
        {
            "customer_id": cust["id"],
            "vehicle_id": veh["id"],
            "description": "Repair minor dent, customer decided not to pursue",
            "category": "cosmetic",
    ...
    post(
        ...
        "/api/tasks",
        {
            "title": "Call Maria Alvarez about coil estimate",
            "notes": "Left a voicemail Tuesday, try again.",
    ...
    post(
        ...
        "/api/tasks",
        {
            "title": "Follow up with parts vendor on AC compressor ETA",
            "assigned_to": ["Ray Ortiz"],
            "due_date": "2026-07-25",
            "urgent": False,
            "order_id": o3["id"],
            "actor": "ui",
    ...
    done_task = post(
        ...
        "/api/tasks",
        {
            "title": "Order shop towels and gloves",
            "assigned_to": ["Lou Grant"],
            "urgent": False,
            "actor": "ui",
        },
    )
    ...
    post(
        client,
        "/api/suggestions",
        {"text": "Add a text-message reminder the day before a promised pickup date.", "author": "Dana Whitfield"},
    )
    resolved = post(
        client,
        "/api/suggestions",
        {"text": "Let us print a work order without pricing for the technician copy.", "author": "Ray Ortiz"},
    )
