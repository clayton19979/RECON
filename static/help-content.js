...
const HELP_TOPICS = [
...

  {
    ...
    view: "updates",
    aliases: ["update", "upgrade", "new version", "version", "out of date", "install update", "newer", "latest", "what version am i on", "check for updates"],
    summary: "The Updates screen says what this computer is running and installs a newer version when there is one — the main computer hands updates to every other PC, so you update it once and the rest follow.",
    steps: [
      "Open Updates from the left-hand menu. 'Running Now' is the version this computer has; the small version number under the shop's name in the corner says the same thing from any screen.",
      "Click 'Check for Updates' to ask whether anything newer has been published. It only downloads — nothing is installed until you ask.",
      "If a version is waiting, it appears under Available with an 'Update Now' button. Click it, and follow the installer. Windows will ask for permission; that's normal, RECON installs for the whole machine.",
      "A bar also appears across the top of RECON whenever an update is ready, so you don't have to go looking. It waits rather than interrupting a ticket you're in the middle of.",
      "Do the main shop PC first. Every other computer then sees the update on its own and offers it the same way.",
